...
| 2026-09-24 | 석박사통합과정 카테고리 추가 — `MembersData.integratedStudents`(id `ip{N}`), Members 페이지 Ph.D.와 M.S. 사이 "Integrated M.S./Ph.D. Students" 섹션. Hoang Hai Nam Nguyen을 phdStudents에서 이동 | src/types/index.ts, src/data/members.json, src/pages/MembersPage.vue, src/pages/AchievementsPage.vue, agents/content-editor.md, agents/schema-guardian.md, references/data-schemas.md | 사용자 요청: "Integrated M.S./Ph.D. Students 만들어줘. nam이 그 프로그램이야" |
| 2026-09-24 | alumni 사진 비게시 규칙 — 졸업 이동 시 `image: ""` + 사진 파일 `git rm`. 기존 alumni(Byeong Se Choi) 사진 삭제 | agents/content-editor.md, references/data-schemas.md, src/data/members.json, public/images/members/choi.JPG | 사용자 요청: "alumni는 그림 게시하지 말자" |
